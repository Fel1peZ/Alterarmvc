...
class {$nomeClasse}Control {
    ...
    function alterar(){
        {$posts}
        \$this->dao->alterar(\$_REQUEST['id'],\$this->{$nomeTabela});
    ...
    function ClassesDao(){
     foreach ($this->tabelas as $tabela) {
         ...
         foreach ($atributos as $atributo) {
             ...
             if ($atributo != $id){
                array_push($vetAtributosAlt,"\${$atributo}");
                array_push($AtributosAlt," {$atributo}=?");
                $AtributosMetodosAlt.="\${$atributo}=\$obj->get{$atr}();\n";
             }
                
         }
         $alterar = implode(",",$AtributosAlt);//lista de atributos sem o id pra passar pro alterar
         $atributosOk=implode(",",$vetAtributos);
         $atributosOkAlt=implode(",",$vetAtributosAlt);
         $conteudo = <<<EOT
<?php
...
function alterar(\$id,\$obj){
    \$sql = "update {$nomeTabela} set{$alterar}  where {$id}=\$id";
    \$stmt = \$this->con->prepare(\$sql);
    {$AtributosMetodosAlt}
